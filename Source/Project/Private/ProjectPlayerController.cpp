...
void AProjectPlayerController::Tick(float DeltaTime) {
    ...
    if (fDrawingEnergy < fDefaultDrawingEnergy) {
        UpdateEnergy(fEnergyCharge * DeltaTime * 100);
    }
...
void AProjectPlayerController::BeginPlay() {
    ...
    if (!FireModelData || !WaterModelData || !TreeModelData || !ElectricModelData)
    {
    ...
    Model = Runtime->CreateModel(FireModelData);
    ...
    if (!RunONNX(
        FPaths::Combine(
            FPaths::ProjectContentDir(),
            TEXT("DrawingImage/Water.png")
        ),
        WaterFeature))
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to generate WaterFeature")); //여기 오류
        return;
    }
    ...
    if (!RunONNX(
        FPaths::Combine(
            FPaths::ProjectContentDir(),
            TEXT("DrawingImage/Tree.png")
        ),
        TreeFeature))
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to generate TreeFeature")); //여기 오류
        return;
    }
    ...
    if (!RunONNX(
        FPaths::Combine(
            FPaths::ProjectContentDir(),
            TEXT("DrawingImage/Electric.png")
        ),
        ElectricFeature))
    {
...
bool AProjectPlayerController::IsSameShape( const FString& PlayerImagePath, float Threshold)
{
    ...
    {
    case EColor::RED:
        Similarity = CosineSimilarity(PlayerFeature, FireFeature);
        UE_LOG(LogTemp, Warning, TEXT("RED"));
        break;
    case EColor::BLUE:
        Similarity = CosineSimilarity(PlayerFeature, WaterFeature);
        UE_LOG(LogTemp, Warning, TEXT("blue"));
        break;
    case EColor::YELLOW:
        Similarity = CosineSimilarity(PlayerFeature, ElectricFeature);
        UE_LOG(LogTemp, Warning, TEXT("yellow"));
        break;
    case EColor::GREEN:
        Similarity = CosineSimilarity(PlayerFeature, TreeFeature);
