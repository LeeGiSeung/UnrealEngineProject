...
def extract_road_shp():
    ...
    for folder_name in os.listdir(ROOT_DIR):
        if os.path.isdir(os.path.join(ROOT_DIR, folder_name)) and folder_name.startswith("2MAP5000_SHP_"):
            ...
            # 도로 데이터는 'SHP_도로_추출' 폴더에 따로 모읍니다.
            output_dir = os.path.join(parent_folder_path, "SHP_도로_추출")
            ...
            for root, dirs, files in os.walk(parent_folder_path):
                ...
                for file in files:
                    ...
                    if file_name in TARGET_FILES and file_ext_lower in SHP_EXTENSIONS:
                        ...
                        # 🔥 중간에 Road_ 네이밍 규칙 추가
                        new_file_name = f"{en_region}_Road_{file_name}{file_ext}"
                        
                        src_file_path = os.path.join(root, file)
                        dst_file_path = os.path.join(output_dir, new_file_name)
                        
                        shutil.copy2(src_file_path, dst_file_path)
                        print(f"   ↳ [도로 복사] {file} -> {new_file_name}")

if __name__ == "__main__":
    print("🚀 [도로 데이터] 추출 및 이름 변경을 시작합니다.")
    extract_road_shp()
    print("\n✅ 도로 데이터 작업이 완료되었습니다.")