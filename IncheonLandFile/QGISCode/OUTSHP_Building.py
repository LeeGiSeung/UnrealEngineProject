...
INTEGRATED_OUTPUT_DIR = os.path.join(ROOT_DIR, "BuildingSHP")
TARGET_FILES = ["N3A_B0010000"] # 건물 대상 파일
...
def extract_building_shp():
    ...
    if not os.path.exists(INTEGRATED_OUTPUT_DIR):
        ...
        print(f"📂 통합 건물 폴더 생성 완료: {INTEGRATED_OUTPUT_DIR}\n")
    
    print("🚀 [건물 데이터] 하위 폴더명 명시형 순차 복사를 시작합니다...")
    print("--------------------------------------------------")
        ...
        for root, dirs, files in os.walk(parent_folder_path):
            ...
            for file in files:
                ...
                if file_name in TARGET_FILES and file_ext.lower() in SHP_EXTENSIONS:
                    ...
                    # ★ 중복 방지를 위해 영문지역명 + 숫자폴더명 + Building 형식을 조립합니다 ★
                    new_file_name = f"{en_region}_{current_subfolder_name}_Building_{file_name}{file_ext}"
...
if __name__ == "__main__":
    ...
    print("\n--------------------------------------------------")
    print(f"✅ 모든 건물 데이터가 중복 없이 BuildingSHP 폴더로 모였습니다!")
